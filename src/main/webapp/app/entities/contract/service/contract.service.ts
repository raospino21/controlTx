...
type RestOf<T extends IContract | NewContract> = Omit<T, 'startTime' | 'finishTime'> & {
...
export class ContractService {
  ...
  protected convertDateFromClient<T extends IContract | NewContract | PartialUpdateContract>(contract: T): RestOf<T> {
    return {
      ...contract,
      startTime: contract.startTime?.toJSON() ?? null,
      finishTime: contract.finishTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restContract: RestContract): IContract {
    return {
      ...restContract,
      startTime: restContract.startTime ? dayjs(restContract.startTime) : undefined,
      finishTime: restContract.finishTime ? dayjs(restContract.finishTime) : undefined,
    };
  }
